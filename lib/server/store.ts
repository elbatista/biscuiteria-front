...
export type StoreSortOption = "featured" | "recent" | "price-asc" | "price-desc";
...
export type StorePageData = {
  ...
  latestCollections: StoreCollectionSummary[];
  selectedCategory: StoreCategorySummary | null;
  selectedCollection: StoreCollectionSummary | null;
...
export function parseStoreSort(value?: string | string[]): StoreSortOption {
...
export async function getStorePageData(input?: {
  categorySlug?: string | string[];
  collectionSlug?: string | string[];
  sort?: string | string[];
}): Promise<StorePageData> {
  const categorySlug = normalizeSingleValue(input?.categorySlug);
  const collectionSlug = normalizeSingleValue(input?.collectionSlug);
  const sort = parseStoreSort(input?.sort);

  const [categories, latestCollections, selectedCategory, selectedCollection] =
    await Promise.all([
