...
* manter consistência entre pedido, pagamento e estoque
...
13. estoque é baixado apenas quando o pagamento é aprovado
14. cliente visualiza o status do pedido
...
* para o MVP, manter o estoque simples
* não usar reserva temporária inicialmente
* a baixa ocorre apenas após pagamento aprovado

## 6.4. Tabela inventory_movements
...
* rastrear entradas e saídas de estoque
* facilitar auditoria e suporte
* simplificar investigação de inconsistências
...
## 6.10. Tabela order_status_history
...
## Status de movimento de estoque
...
5. backend valida disponibilidade de estoque
6. backend cria um snapshot do pedido
7. backend cria o pedido com status `pending_payment`
8. backend cria os itens do pedido
9. backend cria o endereço de entrega
10. backend gera a preferência no Mercado Pago
11. backend salva os identificadores do pagamento
12. backend retorna a URL de checkout
13. frontend redireciona o cliente
...
10. backend baixa o estoque somente se o pagamento estiver aprovado
11. backend registra o movimento de estoque
12. backend marca o evento como processado
...
## 8.6. Fluxo de atualização de estoque

### Regra recomendada

* não baixar estoque ao adicionar ao carrinho
* não baixar estoque ao entrar no checkout
* validar estoque antes de criar o pedido
* validar novamente no momento da aprovação
* baixar estoque somente em pagamento aprovado
* impedir estoque negativo com atualização transacional
...
* leitura do estoque disponível
* bloqueio de compra quando indisponível
...
Toda atualização de estoque deve validar disponibilidade antes da baixa.
...
A baixa de estoque deve:

* validar disponibilidade no momento da aprovação
* ser feita em transação
...
* atualizar pedido
* registrar histórico
* baixar estoque somente quando aplicável
...
* evento duplicado
* atualização duplicada de estoque
...
* logs de erro ao atualizar estoque
...
* cada movimentação de estoque deve ser auditável
...
* estoque baixado apenas após aprovação do pagamento
